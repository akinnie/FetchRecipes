...
""".data(using: .utf8)
    ...
    func recipeDTOIsCreatedWithValidData() throws {
        ...
        let recipes = try sut.decodeRecipesFromData(recipesValidJsonData)
        #expect(recipes != nil)
    }
    ...
    @Test
    func recipeListIsNotEmpty() throws {
        let sut = NetworkManager()
        let recipes = try sut.decodeRecipesFromData(recipesValidJsonData)
    ...
    @Test
    func emptyRecipeListDoesNotThrowError() throws {
        let sut = NetworkManager()
        let recipes = try sut.decodeRecipesFromData("{\"recipes\":[]}".data(using: .utf8))
        #expect(recipes != nil)
    }
